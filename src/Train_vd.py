...
if retrieval:
    ...
    if not os.path.exists(file_path):
        ...
        for i in tqdm(range(len(train_images) // emb_batch_size), desc="Encoding images..."):
            retrieval_image_train[i * emb_batch_size:i * emb_batch_size + emb_batch_size] = clip_extractor.embed_image(train_images[i * emb_batch_size:i * emb_batch_size + emb_batch_size]).to("cpu")
...
if dual_guidance:
    clip_text_train = clip_text_train[:len(train_captions)]
if blurry_recon:
    vae_image_train = vae_image_train[:len(train_images)]
if retrieval:
