...
class BrainNetwork(nn.Module):
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=True, text_clip=False, clip_scale=1, clip_seq_dim_retrieval=257, text_clip_dim = 1280, clip_embedding_dim=768, vdvae_dim = 91168):
        super().__init__()
        print('modified 2')
        ...
        self.clip_proj_1 = self.projector(out_dim, clip_embedding_dim, h=clip_embedding_dim) 
        # self.inital_rr = self.projector(in_dim, h, h=h)
        
        if text_clip:
            self.backbone_linear_txt = nn.Linear(h * seq_len, 77*clip_size, bias=True) 
            self.clip_txt_proj = self.projector(clip_size, text_clip_dim, h=text_clip_dim)
        if self.blurry_recon:
            self.projection_blurry = self.projector(h*seq_len, vdvae_dim, h=clip_size)
            ...
            # )
        self.linear_blurry = nn.Sequential(
            nn.Linear(h, vdvae_dim)
        )
    ...
    def forward(self, x):
        ...
        if self.clip_scale>0:
            ...
            c = self.clip_proj_1(c.reshape(len(backbone), -1)).unsqueeze(1)
            if self.text_clip:
                c_txt = self.clip_txt_proj(backbone_txt)

        if self.blurry_recon:
            b = self.linear_blurry(raw_input[:,0,:])
            # b = self.blin1(x)
            # b = self.bdropout(b)
